...
public class OrderFinanceService extends BaseService {
	...
	public ResultPageDTO<OrderFinanceDO> page(OrderFinanceDO orderFinance, Integer pageNum, Integer pageSize) {
		...
		for (OrderFinanceDO orderFinanceDO : pageList) {
			if (null != orderFinanceDO.getCustomerId()) {
				CustomerDO customerDO = customerDAO.getById(orderFinanceDO.getCustomerId());
				if (null != customerDO) {
					orderFinanceDO.setCustomerName(customerDO.getName());
					orderFinanceDO.setCustomerPhone(customerDO.getMobile());
				}
			}

			if (null != orderFinanceDO.getCityId()) {
