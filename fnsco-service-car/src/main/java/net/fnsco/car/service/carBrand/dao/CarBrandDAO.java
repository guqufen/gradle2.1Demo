...
public interface CarBrandDAO {
	...
	@Results({ @Result(column = "id", property = "id"), @Result(column = "name", property = "name"),
			@Result(column = "supper_id", property = "supperId"), @Result(column = "level", property = "level"),
			@Result(column = "icon_img_path", property = "iconImgPath"), @Result(column = "model", property = "model"),
			@Result(column = "childCount", property = "childCount"), @Result(column = "is_hot", property = "isHot") })
	@Select("select r.*, (select count(*) from car_dic_type t where t.supper_id = r.id) as childCount from car_dic_type r where level='1' order by CONVERT(name USING gbk) asc")
	...
	@Results({ @Result(column = "id", property = "id"), @Result(column = "name", property = "name"),
			...
			@Result(column = "icon_img_path", property = "iconImgPath"), @Result(column = "model", property = "model"),
			@Result(column = "childCount", property = "childCount"), @Result(column = "is_hot", property = "isHot") })
	@Select("select r.*, (select count(*) from car_dic_type t where t.supper_id = r.id) as childCount from car_dic_type r where supper_id = #{id} order by id")
