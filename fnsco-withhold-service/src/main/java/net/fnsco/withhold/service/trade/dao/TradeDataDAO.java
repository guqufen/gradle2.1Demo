...
public interface TradeDataDAO {
    ...
    @Insert("INSERT into t_trade_data(id,withhold_id,txn_amt,status,fail_reason,user_name,mobile,bank_card,sub_bank_name,an_bank_id,account_type,txn_type,txn_sub_type,biz_type,access_type,mer_id,order_sn,txn_time,currency,back_url,pay_type,subject,body,customer_info,purpose,resp_code,resp_msg,certif_type,certify_id) VALUES (#{id},#{withholdId},#{txnAmt},#{status},#{failReason},#{userName},#{mobile},#{bankCard},#{subBankName},#{anBankId},#{accountType},#{txnType},#{txnSubType},#{bizType},#{accessType},#{merId},#{orderSn},#{txnTime},#{currency},#{backUrl},#{payType},#{subject},#{body},#{customerInfo},#{purpose},#{respCode},#{respMsg},#{certifType},#{certifyId})")
