...
public class ReportService extends BaseService {
	...
	public ResultPageDTO<ReportInfoDO2> page2(ReportInfoDO2 reportInfoDO, Integer pageNum, Integer pageSize) {
		...
		reportInfoDO.setAgentId(userDo.getAgentId());
		// 获取该产品下的还款能力值
		if (reportInfoDO.getProductId() != null) {
			ProductDO productDO = productDAO.getById(reportInfoDO.getProductId());
			reportInfoDO.setPayAbility(productDO.getPayAbilityMin());
	 ...
	 * 
	 * @param reportInfoDO
	 *            status:10-已生成；20-未生成/已生成，在表中且有效；未生成-不在表中
	 * @param pageNum
	 * @param pageSize
	 * @return
	 */
	...
	public ResultDTO queryTradingVolumeReport(String entityInnerCode, Integer merchantId) {
		...
		List<Map.Entry<String, ReportBusiness>> lists = new ArrayList<Map.Entry<String, ReportBusiness>>(
				allMap.entrySet());
		Collections.sort(lists, new Comparator<Map.Entry<String, ReportBusiness>>() {
			// 升序排序
			public int compare(Entry<String, ReportBusiness> o1, Entry<String, ReportBusiness> o2) {
	...
	/*
	 * private String handleDateYear(Date date, Integer num) { Calendar calendar
	 * = Calendar.getInstance(); calendar.setTime(date);
	 * calendar.add(Calendar.MONTH, num); SimpleDateFormat sdf = new
	 * SimpleDateFormat("YY-MM"); String dateStr =
	 * sdf.format(calendar.getTime()); return dateStr; }
	 */
		...
		TaskUtils.execAsyn(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					sb.append("<div style='font-size:14px;margin-top:50px;line-height:12px;'><p>dear:</p><p>"
							+ dto.getName() + "申请生成关于" + reportInfoDO.getMerName() + "的风控报告,请尽快处理!</p>"
		...
		TaskUtils.execAsyn(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					StringBuffer sb = new StringBuffer();
					sb.append("<div style='font-size:14px;margin-top:50px;line-height:12px;'><p>dear:</p><p>"
							+ dto.getName() + "</p><p>关于" + reportInfoDO.getMerName() + "的'风控+'报告已经生成!</p><p>点击查看"
	...
	public ResultPageDTO pageRepay(Integer id, Integer pageNum, Integer pageSize) {
		...
		List<ReportRepaymentHistoryDO> pageList = this.reportRepaymentHistoryDAO.pageList(reportRepaymentHistory,
				pageNum, pageSize);
		for (ReportRepaymentHistoryDO reportRepaymentHistoryDO : pageList) {
			reportRepaymentHistoryDO
					.setLastModifyTimeStr(DateUtils.dateFormatToStr(reportRepaymentHistoryDO.getLastModifyTime()));
		}
