...
public class WithholdInfoService extends BaseService {
    ...
    // 添加
    public ResultDTO doAdd(WithholdInfoDO withholdInfo, int loginUserId) {
        logger.info("开始添加WithholdInfoService.add,withholdInfo=" + withholdInfo.toString());
        //判断是否支持该银行并且每次扣款额度是否正确  getByCardNum  
        if (!Strings.isNullOrEmpty(withholdInfo.getBankCard()) || withholdInfo.getAmount() != null) {
