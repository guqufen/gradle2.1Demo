...
public class MerchantInfoE789Controller extends BaseController {
    ...
    public ResultDTO<MerchantInfoVO> queryBalance(@RequestBody CommonJO commonJO) {
		...
		if(null != merchantEntity) {
			resultVO.setBusinessLicenseNum(StringUtil.replaceString(4,4,merchantEntity.getBusinessLicenseNum()));
			resultVO.setLegalPerson(merchantEntity.getLegalPerson());
			resultVO.setLegalPersonMobile(StringUtil.replaceString(3,4,merchantEntity.getLegalPersonMobile()));
			resultVO.setMerName(merchantEntity.getMercName());
			resultVO.setCardNum(StringUtil.replaceString(4,4,merchantEntity.getCardNum()));
		...
		if(null != merchantBank) {
			...
			resultVO.setAccountNo(StringUtil.replaceString(0,4,accountNo));
		}
